import type { GalleryItemCard, GalleryItemDetail, PublishGalleryPayload, PublishGalleryResponse } from './types';
...
export function getMockGalleryList(): { items: GalleryItemCard[]; total: number; nextPage: null } {
  return {
    ...
    nextPage: null,
    items: mockItems.map(({ pattern, sourceUrl: _sourceUrl, exportUrl: _exportUrl, stats, ...item }) => ({
      ...item,
      patternSummary: {
        width: pattern.width,
        height: pattern.height,
        beadCount: pattern.beadCount,
        paletteCount: pattern.paletteCount,
      },
      stats: {
