...
- 第三轮继续处理普通页面体系内的剩余高收益页面，先收 `/workshop/inventory` 与 `/collection/:itemId`，暂不进入编辑器和专注模式这类 fullscreen 工具页。
...
- 不重构 `/workshop/editor/:projectId` 和 `/workshop/focus/:projectId`，它们进入第四轮 fullscreen 工具页专项。
